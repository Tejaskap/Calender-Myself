...
const displayEventTimes = (events) => {
  if (events.length > 0) {
    return events.map((event, index) => (
      <div key={index}>
        <p>Event {index + 1}:</p>
        <p>Start Time: {new Date(event.start.dateTime).toLocaleTimeString()}</p>
        <p>End Time: {new Date(event.end.dateTime).toLocaleTimeString()}</p>
        <hr />
      </div>
    ));
  } else {
    return <p>No events found in the response.</p>;
  }
};

export default displayEventTimes;
